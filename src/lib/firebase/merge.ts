import type { SyncPayload } from './serializer.js';
...
export function mergeState(
	localData: { _syncedAt: number; data: SyncPayload } | null,
	cloudData: { updatedAt: number | { toMillis: () => number }; data: SyncPayload } | null
): { changed: boolean; data: SyncPayload | null } {
	...
	const cloudTime =
		typeof cloudData.updatedAt === 'object' && 'toMillis' in cloudData.updatedAt
			? cloudData.updatedAt.toMillis()
			: (cloudData.updatedAt as number) || 0;
