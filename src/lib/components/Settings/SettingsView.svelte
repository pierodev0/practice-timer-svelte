<script lang="ts">
	...
	function showArchivedExercises() {
		...
		const list = archived
			.map((ex, i) => `${i + 1}. ${ex.title} (${ex.bpm} BPM)`)
			.join('\n');
	...
	function exportAllData() {
		...
		downloadJSON(
			JSON.stringify(
				{ routines: state.routines, stats: state.stats, sessions: state.sessions },
				null,
				2
			),
...
<div class="view-section active">
	...
	<div class="p-4 space-y-4 pb-12">
			...
			<div class="flex items-center justify-between">
				<div class="flex items-center gap-3">
					...
					<div>
						...
						<p class="text-xs text-gray-400" id="settings-archived-count">
							{archivedCount} ejercicio{archivedCount !== 1 ? 's' : ''}
						</p>
					</div>
				</div>
				<button
					type="button"
					id="settings-archived-btn"
					class="text-sm text-[#E53935] font-medium hover:underline"
					onclick={showArchivedExercises}
				>
					Ver
				</button>
			</div>
		...
		<div class="card p-4">
			...
			<div class="space-y-3">
				...
				>
					...
					<div>
						<p class="font-medium text-gray-800 text-sm">Copias en la Nube</p>
						<p class="text-xs text-gray-400">Guardar y restaurar desde Firestore</p>
					</div>
				</button>
			</div>
		</div>
		...
		<!-- Cloud Sync -->
		<SyncSection {currentUser} {syncStatus} {lastSyncTime} />
