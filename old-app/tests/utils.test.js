...
// We need to mock nanoid so IDs are deterministic in tests
vi.mock('nanoid', () => ({
  nanoid: () => 'test-id-123',
}));
...
describe('sanitizeImportedRoutine', () => {
  ...
  it('handles array input by wrapping in an array check (the function wraps single obj)', () => {
    // sanitizeImportedRoutine expects a single routine object, not array
    // The array handling is done by importRoutines in routines.js
    const result = sanitizeImportedRoutine({
      name: 'Single',
      exercises: [],
    });
